...
<script>
import { getById, updateAdmin } from '@/api/admin'
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          updateAdmin(this.formValidate).then(({code,data,message}) => {
            if(code === 200){
    ...
    getAdminById () {
      ...
      getById({ id }).then(({ code, data, message }) => {
        ...
        this.accessMenuList = this.formValidate.accessMenu.split(';')
        delete this.formValidate.createTime
        delete this.formValidate.updateTime
        delete this.formValidate.menuList
        delete this.formValidate.permissionList
      }).catch(err => {
